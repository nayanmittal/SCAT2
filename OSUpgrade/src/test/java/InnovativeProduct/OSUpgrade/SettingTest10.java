...
/*	Handled message popup like cancel,mode description ,accept permission,click ok.
...
public class SettingTest10 extends BaseSetting2
{
	...
	String slastt=""; // third last element to be removed if the list size is greater than 10.
	 ...
	 String[] skipMenu={"Notifications" ,"Biometrics and security","Wallpaper","Connections","Sounds and vibration","Notifications","Display","Lock screen","Privacy","Location","Wallpapers and themes","Wallpaper and themes","User Manual","Contact us","Themes","Biometrics security patch","Auto-fill service from Google","Hide apps","Game Launcher","Google Location History",
			 ...
			 "Ultra data saving mode","Smart pop-up view","Left shortcut","Right shortcut","App permissions","Permission manager","Open app"}; //Ultra data saving mode, need to check not able to take dump of it.//Permission manager-no need to get the lists of permission.
	...
	public void Menu1() throws InterruptedException, IOException
	{
		...
		boolean matcher2=false;
			...
			//System.out.println("Before click/New Depth last element is  "+Lbefore);
		    		...
		    		String THIRD="";
				     ...
				     {
				      //e.printStackTrace();
			    	 System.out.println("Less than 3 element before click  ");
				     }
		...
		try 
		{
			if(s3.size()>0)
			{
		    	...
		    	//clicking starts here// && !menu3.equalsIgnoreCase("Access Point Names")//App permissions //Smart pop-up view
		    	if(depth<3 && !(menu1.equalsIgnoreCase("Apps") && depth==1) && !(menu1.equalsIgnoreCase("Google") && depth==1) && !(menu1.equalsIgnoreCase("Notifications") && depth==2) && !(menu2.equalsIgnoreCase("Install unknown apps") && depth==2) && !(menu2.equalsIgnoreCase("App shortcuts") && depth==2) &&  !(menu2.equalsIgnoreCase("Install unknown apps") && depth==2) && !(menu3.equalsIgnoreCase("Notification reminders")) && !(menu2.equalsIgnoreCase("Send SOS messages") && depth==2))   
		    	{
		    		boolean cond=false;
		    	 	try
		    		{	 
		    		...
		    		// if getAttribute method dosn't find any value then it return null and if you perform any operation on that then null pointer excecption is thrown .//com.samsung.android.lool:id/title_text
					...
					*/
					...
					//if size is Diff. but last element is same then depth not changed.
							if(el_count)
							{
								...
								TouchAction T=new TouchAction(driver);
					    		T.tap(tapOptions().withElement(element(ae))).perform();
					    		Thread.sleep(500);// Don't remove this wait its very important otherwise Cancle will not get clicked.
					    		handleKeyboard();
					    		pressCancel();
					    		pressOk();
					    		pressLater();
					    		}
								else if(THIRD.equalsIgnoreCase("On") || THIRD.equalsIgnoreCase("Off"))
								{
...
*/
	...
	public void acceptPermissions()
	{
		...
		//don't change below condition to while loop otherwise it will create problem.
		if(driver.findElementsByXPath("//*[@text='Allow only while using the app']").size()>0)
		driver.findElementsByXPath("//*[@text='Allow only while using the app']").get(0).click();
		/*
		try
		{
	...
	public void printFunction(AndroidElement ae) throws IOException
	{
					...
					{   
						...
						if("android.widget.CheckedTextView".equalsIgnoreCase(ae.getAttribute("className")))
						{
						if("true".equalsIgnoreCase(ae.getAttribute("checked")))
						{
						...
						else if(ae.isEnabled() && ("android:id/title".equalsIgnoreCase(ae.getAttribute("resourceId")) || "com.sec.android.app.camera:id/spinner_title".equalsIgnoreCase(ae.getAttribute("resourceId")) ))
						{
						//System.out.println("------------------------");
						System.out.println("\n->"+ae.getText());
						text="\n"+text;
						...
						else
						{
							...
							else if(!", Header".equalsIgnoreCase(ae.getAttribute("contentDescription")) && ae.getAttribute("contentDescription").length()!=0) // this null comparison have some problem .because its basically allow all cont-descp whether it blank(may be it is returning blank not null) or not
							{
	...
	public void Test2()
	{
	System.out.println("Test case just written to check the functionality of testng");	
	}
